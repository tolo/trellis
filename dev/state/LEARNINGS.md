...
- **`Directory.watch(recursive: true)` is not supported on Linux** (dart:io docs: inotify; only macOS/Windows honour `recursive`). A watcher test for nested paths passes on the maintainer's Mac and times out deterministically on ubuntu CI — that is a platform gap, not a flake (TD-013). Guard such tests with `skip: Platform.isLinux ? '<reason; TD>' : false` and file the product limitation, rather than raising the timeout.
