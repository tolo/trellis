...
- A template directory the OS refuses to watch — most often Linux's `fs.inotify.max_user_watches` limit on a large template tree — now prints one warning naming the directory, instead of capping hot reload silently.
