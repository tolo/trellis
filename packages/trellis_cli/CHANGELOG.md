...
- The Shelf and Dart Frog scaffold layouts set the CSRF header on both the HTMX 2 (`htmx:configRequest`) and the
  HTMX 4 (`htmx:config:request`) request-configuration event, so a generated project keeps working when its HTMX
  script tag is moved to 4.x. The pinned HTMX version stays 2.0.10 (ADR-011).

### Fixed

- `trellis create` scaffolds failed `dart pub get` out of the box: every pubspec template pinned the Trellis satellites
  (`trellis_shelf`, `trellis_dev`, `trellis_dart_frog`, `trellis_relic`, `trellis_site`) at `^0.1.0` and `trellis` at
  `^0.8.0`, but the satellites were first published at 0.8.1, so pub reported "doesn't match any versions". The
  constraints now derive from the CLI's own version, so a project generated by trellis_cli X.Y.Z depends on `^X.Y.Z`
  and `tool/version_lockstep.sh` keeps it current. `scaffold_pubspec_constraints_test.dart` asserts each scaffold's
  `trellis*` constraints admit that version; the e2e suites never caught this because they override the Trellis
  dependencies with workspace paths before resolving.
