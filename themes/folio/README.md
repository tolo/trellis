...
controlled only by `accent_color`. `logo` takes a prefix-relative image path and replaces the built-in sprout mark in
the header; leave it unset to keep the mark.
...
`caption` labels the herbarium sheet. `image` mounts your own specimen on the sheet — `src` is prefix-relative, and
omitting the key keeps the drawn engraving. Each `ctas` entry needs `label` and `href`; `style: secondary` renders the
outlined variant, anything else renders the filled one. Root-relative `href` values are joined with `pathPrefix`.
